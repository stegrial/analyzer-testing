  ...
  feature 'AT-98 click on new step  (Create Component Process with Step type and with further removal)' do
    ...
    scenario 'Recording IL', il: true do
      ...
      step "Admin click on new Component Processes" do
        apps_page.click_on_new_component_processes :il
      end

      step "Admin click on the button Create new" do
        apps_page.create_new_component_processes :il
      end

      step "Admin fills the process name field", "process_test_name" do |processname|
        apps_page.fill_process_name_field :il,processname
      end

      step "Admin click on dropdown" do
        apps_page.click_on_dropdown_deploy_undeploy :il
      end

      step "Admin select on dropdown Undeploy" do
        apps_page.click_on_dropdown_undeploy :il
      end

      step "Admin clicks on dropdown Select Workspace" do
        apps_page.click_on_dropdown_select_workspace :il
      end

      step "Admin select on dropdown Default" do
        apps_page.click_default_dropdown_select_workspace :il
      end

      step "Admin confirm new Component Process" do
        apps_page.confirm_new_component_process :il
      end
