...
module StageCom
  ...
  def click_humburger_menu(ep = false)
    return find(:xpath, ta(@humburger_menu_ta)).click if ep
    find(:xpath, ta(@humburger_menu_ta, @humburger_menu_il)).click
  end

  def click_signin_link(ep = false)
    return find(:xpath, ta(@signin_link_ep)).click if ep
    find(:xpath, ta(@signin_link_ta, @signin_link_il)).click
  end

  def click_account_email(email, ep = false)
    return find(:xpath, ta(@account_email_ep)).click if ep
    find(:xpath, ta(@account_email_ta, @account_email_il)).set(email)
  end

  def click_pass (pass, ep = false)
    return find(:xpath, ta(@pass_ep)).set(pass) if ep
    find(:xpath, ta(@pass_ta, @pass_il)).set(pass)
  end

  def click_signin_btn(ep = false)
    return find(:xpath, ta(@signin_btn_ep)).click if ep
    find(:xpath, ta(@signin_btn_ta, @signin_btn_il)).click
  end

  def click_logo(ep = false)
    return find(:xpath, ta(@logo_ep)).click if ep
    find(:xpath, ta(@logo_ta, @logo_il)).click
  end

  def click_minicart(ep = false)
    return find(:xpath, ta(@minicart_ep)).click if ep
