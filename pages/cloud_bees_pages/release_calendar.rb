...
class CloudBeesReleasesCalendar < PageExtension
  ...
  def click_to_timezone_dropdown(key = nil)
    find_element(key, il_type: :css, tl: SELECT_USER_TIMEZONE_TA, il: SELECT_USER_TIMEZONE_IL).click
  end

  def set_timezone_search_value(key = nil, value)
    find_element(key, il_type: :xpath, tl: SEARCH_TIMEZONE_DROPDOWN_TA, il: SEARCH_TIMEZONE_DROPDOWN_IL).set(value)
  end

  def select_timezone(key = nil, timezone)
    find_element(key, il_type: :xpath, tl: timezone_name(:tl, timezone), il: timezone_name(:il, timezone)).click
  end

  def click_ok_button(key = nil)
    find_element(key, il_type: :css, tl: OK_BUTTON_MODAL_TA, il: OK_BUTTON_MODAL_IL).click
  end

  def click_week_tab_button(key = nil)
    find_element(key, il_type: :css, tl: WEEK_TAB_TA, il: WEEK_TAB_IL).click
  end

  def click_day_tab_button(key = nil)
    find_element(key, il_type: :css, tl: DAY_TAB_TA, il: DAY_TAB_IL).click
  end

  def click_month_tab_button(key = nil)
    find_element(key, il_type: :css, tl: MONTH_TAB_TA, il: MONTH_TAB_IL).click
  end
end