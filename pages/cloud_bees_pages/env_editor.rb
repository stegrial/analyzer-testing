...
class CloudBeesEnvEditor < PageExtension
  ...
  def click_resource_tier(key = nil)
    find_element(key, il_type: :xpath, tl: RESOURCE_TIER_TA, il: RESOURCE_TIER_IL, check_path: $check_path).click
  end

  def click_to_add_resource(key = nil)
    find_element(key, il_type: :css, tl: ADD_RESOURCE_BTN_TA, il: ADD_RESOURCE_BTN_IL, check_path: $check_path).click
  end

  def select_resource_from_list(key = nil)
    find_element(key, il_type: :xpath, tl: SELECT_RESOURCE_FROM_LIST_TA, il: SELECT_RESOURCE_FROM_LIST_IL, check_path: $check_path).click
  end

  def click_map_to_app_hierarchy_menu(key = nil)
    find_element(key, il_type: :css, tl: MAP_TO_APP_BTN_TA, il: MAP_TO_APP_BTN_IL, check_path: $check_path).click
  end

  def set_app_in_search(key = nil, appName)
    find_element(key, il_type: :css, tl: APPNAME_IN_SEARCH_FIELD_TA, il: APPNAME_IN_SEARCH_FIELD_IL, check_path: $check_path).set(appName)
  end

  def select_app_from_maplist(key = nil)
    find_element(key, il_type: :css, tl: APPS_NAME_IN_MAPPING_MODAL_TA, il: APPS_NAME_IN_MAPPING_MODAL_IL, check_path: $check_path).click
  end

  def click_mapping_select_btn(key = nil)
    find_element(key, il_type: :css, tl: MAPPING_SELECT_BTN_TA, il: MAPPING_SELECT_BTN_IL, check_path: $check_path).click
  end

  def click_to_select_tier_for_map(key = nil)
    find_element(key, il_type: :css, tl: TIER_MAPPING_POPOVER_TA, il: TIER_MAPPING_POPOVER_IL, check_path: $check_path).click
  end

  def click_to_close_to_close_modal(key = nil)
    find_element(key, il_type: :css, tl: CLOSE_BTN_TA, il: CLOSE_BTN_IL, check_path: $check_path).click
  end
