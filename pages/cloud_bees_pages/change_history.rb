...
class CloudBeesChangeHistory < PageExtension
  ...
  def click_second_item_in_table(key = nil)
    find_element(key, il_type: :xpath, tl: SECOND_ITEM_IN_TABLE_TA, il: SECOND_ITEM_IN_TABLE_IL).click
  end

  def click_dropdown_last_changes(key = nil)
    find_element(key, il_type: :xpath, tl: DROPDOWN_LAST_CHANGES_TA, il: DROPDOWN_LAST_CHANGES_IL).click
  end

  def click_past_24_hours(key = nil)
    find_element(key, il_type: :xpath, tl: PAST_24_HOURS_TA, il: PAST_24_HOURS_IL).click
  end

  def click_dropdown_past_24_hours(key = nil)
    find_element(key, il_type: :xpath, tl: DROPDOWN_PAST_24_HOURS_TA, il: DROPDOWN_PAST_24_HOURS_IL).click
  end

  def click_between_btn(key = nil)
    find_element(key, il_type: :xpath, tl: BETWEEN_BTN_TA, il: BETWEEN_BTN_IL).click
  end

  def fill_hours_time(key = nil, hours_time)
    find_element(key, il_type: :xpath, tl: SET_HOURS_TIME_TA, il: SET_HOURS_TIME_IL).set(hours_time)
  end

  def fill_minutes_time(key = nil, minutes_time)
    find_element(key, il_type: :xpath, tl: SET_MINUTES_TIME_TA, il: SET_MINUTES_TIME_IL).set(minutes_time)
  end

  def confirm_time(key = nil)
    find_element(key, il_type: :xpath, tl: CONFIRM_TIME_TA, il: CONFIRM_TIME_IL).click
  end

  def fill_and_hours_time(key = nil, hours_time)
    find_element(key, il_type: :xpath, tl: AND_SET_HOURS_TIME_TA, il: AND_SET_HOURS_TIME_IL).set(hours_time)
  end

  def fill_and_minutes_time(key = nil, minutes_time)
    find_element(key, il_type: :xpath, tl: AND_SET_MINUTES_TIME_TA, il: AND_SET_MINUTES_TIME_IL).set(minutes_time)
  end

  def and_confirm_time(key = nil)
    find_element(key, il_type: :xpath, tl: AND_CONFIRM_TIME_TA, il: AND_CONFIRM_TIME_IL).click
  end

  def click_search_btn(key = nil)
    find_element(key, il_type: :xpath, tl: SEARCH_BTN_TA, il: SEARCH_BTN_IL).click
  end

  def click_last_changes_dropdown(key = nil)
    find_element(key, il_type: :xpath, tl: LAST_CHANGES_DROPDOWN_TA, il: LAST_CHANGES_DROPDOWN_IL).click
  end

  def click_past_3_days(key = nil)
    find_element(key, il_type: :xpath, tl: PAST_3_DAYS_TA, il: PAST_3_DAYS_IL).click
  end

  def click_all_dropdown(key = nil)
    find_element(key, il_type: :xpath, tl: ALL_DROPDOWN_TA, il: ALL_DROPDOWN_IL).click
  end

  def click_all_object(key = nil)
    find_element(key, il_type: :xpath, tl: ALL_OBJECTS_TA, il: ALL_OBJECTS_IL).click
  end

  def click_application(key = nil)
    find_element(key, il_type: :xpath, tl: APPLICATION_TA, il: APPLICATION_IL).click
  end

  def click_component(key = nil)
    find_element(key, il_type: :xpath, tl: COMPONENT_TA, il: COMPONENT_IL).click
  end

  def click_on_search_field(key = nil)
    find_element(key, il_type: :xpath, tl: SEARCH_FIELD_TA, il: SEARCH_FIELD_IL).click
  end
